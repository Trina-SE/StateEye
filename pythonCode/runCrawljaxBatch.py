...
######################## REGRESSION UTILS ##################
def restartDockerVersion(appName, version=0):
	# if version == None:
	# 	restartDocker(getDockerName(appName))
	# 	return
	# dockerList = getDockerList(version)
	# print(dockerList[appName])
	restartDocker(getDockerName(appName=appName, version=version))
...
def restartDocker(dockerName):
	...
	# evalDocker = ["eval $(docker-machine env default)"]
	# try:
	# 	check_call(evalDocker)
	# except CalledProcessError as ex:
	# 	print("Eval docker not needed? ")
	# 	print(ex)
	if(dockerName=='dbApps'):
		...
		startDocker = ['docker-compose', '-f', yml, 'up', '-d', '--force-recreate']
		try:
	...
	# startDocker = ['/Users/fraggen/git/DIG_FSE2019/fse2019/'+ dockerName +'/run-docker.sh']
...
def cleanup(appName = None, version=0):
	...
	if not dockerName is None:
		if(dockerName=='dbApps'):
			yml = os.path.join(DOCKER_LOCATION,  dockerName ,'docker-compose.yml')
			startDocker = ['docker-compose', '-f', yml, 'down']
			try:
				check_call(startDocker)
				sleep(30)
			except CalledProcessError as ex:
...
def startProcess(command, outputPath="output_crawljax_" + str(datetime.now().strftime("%Y%m%d-%H%M%S")) + ".log", changeDir=True,
				 DEBUG=False):
	changed = False
	current = os.getcwd()
	try:
...
def runBestCrawls(RUNTIME = 30):
	BASE_COMMAND=['java', '-jar', '/home/fraggen/fraggen/git/art-fork/crawljax/examples/target/crawljax-examples-3.7-SNAPSHOT-jar-with-dependencies.jar']
	# RUNTIME = 30
	succesful = []
	unsuccesful = []
	skipped = []
...
def runAllAlgos():
	...
	# excludeAlgos=[]
	excludeAlgos = ['DOM_SIMHASH', 'DOM_CONTENTHASH','DOM_LEVENSHTEIN', 'DOM_RTED', 'VISUAL_PDIFF', 'VISUAL_SSIM', 'VISUAL_PHASH', 'VISUAL_HYST','VISUAL_BLOCKHASH', 'VISUAL_SIFT']
...
def runAlgo(appName, algo, runtime, threshold = -1, logFile = os.path.join( "logs", "crawljaxLog_" + str(datetime.now().strftime("%Y%m%d-%H%M%S"))+".log"), maxStates = -1, rerun=False):
	...
	existingCrawlData = getExistingCrawl(appName, algo, threshold, runtime)
...
excludeApps=['petclinic', 'claroline', 'dimeshift', 'pagekit', 'phoenix', 'ppma','mantisbt']

BASE_COMMAND_HYBRID=['java', '-jar', '/home/fraggen/fraggen/jars/crawljax-examples-4.0-beta-jar-with-dependencies.jar']

BASE_COMMAND=['java', '-jar', '/home/fraggen/fraggen/jars/crawljax-examples-3.7-SNAPSHOT-jar-with-dependencies.jar']
...
if __name__ == "__main__":
	...
	runBestCrawls(RUNTIME = 60)
	# runAllAlgos()
	# status, command = runAlgo("petclinic", "DOM_RTED", 5, 0.0)
	# print(status + str(command))
	# runAlgo("petclinic", "default", 60)
	# runAlgo("claroline", "default", 60)
	# runAlgo("addressbook", "dom_levenshtein", 1, threshold=0.002)
	# runAlgo('claroline', 'VISUAL_PDIFF', '5', threshold=0.009)
	# runAlgo('dimeshift', 'DOM_RTED', 60, threshold=0.0, maxStates = 150)
	# runAlgo('pagekit', 'default', 60, maxStates= 150)
	# runAlgo('pagekit', 'VISUAL_HYST', 60, maxStates= 150)
	# runAlgo('phoenix', 'default', 60, maxStates= 150)
	# runAlgo('retroboard', 'default', 60, maxStates= 150)
	# runAlgo('splittypie', 'rted', 60, maxStates= 150)
	# runAlgo('ppma', 'default', 60, maxStates= 150)
	# runAlgo('ppma', 'VISUAL_SIFT', '1', threshold= '92.7778')
	# runAlgo('mrbs', 'default', 60, maxStates= 150)
	# runAlgo('mantisbt', 'default', 60, maxStates= 150)
	# runAlgo('collabtive', 'VISUAL_HYST', 60, threshold = 0.0, maxStates= 150)
