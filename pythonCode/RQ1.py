...
def testDummyClassifier():
	...
	dbName = "/Users/fraggen/VisCrawler/Test/gt10/gt10_last500Responses.db"
	connectToDB(dbName)
	...
	dbName="/Users/fraggen/git/abstract-state-function-project/src/main/resources/GoldStandards/gs.db"
	connectToDB(dbName)
	allEntries = fetchAllNearDuplicates("where human_classification>=0")
	closeDBConnection()
	y_actual = []
...
def getF1_SAF_allrows(allEntries):
	...
	# 			y_pred[algoStr][0].append(cpred)
	# 			y_pred[algoStr][1].append(npred)
	# 			index = index+1
	# 		if entry[index] == 0 or entry[index] ==1:
	# 			y_actual.append(0)
	# 		else:
	...
	# 	for algo in ALGOS:
	# 		algoStr = str(algo).split('.')[1].upper()
	# 		cm = metrics.confusion_matrix(y_actual, y_pred[algoStr][0])
	# 		# print(cm)
	# 		precision, recall, f1, support = metrics.precision_recall_fscore_support(y_actual, y_pred[algoStr][0], average=None)
	# 		# precision = metrics.precision(y_actual, y_pred[algoStr], average="macro")
	# 		# recall = metrics.recall(y_actual, y_pred[algoStr], average="macro")
	# 		# f1 = metrics.f1_score(y_actual, y_pred[algoStr], average="macro")
	# 		row_c = {'thresholdSet' : threshold_set_name, 'algoName':algoStr, 'c-thre':cloneThresholds[algoStr],'n-thre':-1,'precision':precision[1], 'recall':recall[1], 'f1':f1[1]}
	# 		algoScoreRows.append(row_c)
	# 		cm = metrics.confusion_matrix(y_actual, y_pred[algoStr][1])
	# 		precision, recall, f1, support = metrics.precision_recall_fscore_support(y_actual, y_pred[algoStr][1], average=None)

	# 		row_n = {'thresholdSet' : threshold_set_name, 'algoName':algoStr, 'c-thre':-1,'n-thre':ndThresholds[algoStr],'precision':precision[1], 'recall':recall[1], 'f1':f1[1]}

	# 		algoScoreRows.append(row_n)

	# 	X = [[0]] * len(y_actual)
	# 	dummyClassifier.fit(X, y_actual)
	# 	y_pred_dummy = dummyClassifier.predict(y_actual)
	# 	precision, recall, f1, support = metrics.precision_recall_fscore_support(y_actual, y_pred_dummy, average=None)
	# 	row2 = {'thresholdSet' :None, 'algoName':"dummy", 'c-thre':None,'n-thre':None,'precision':precision[1], 'recall':recall[1], 'f1':f1[1]}
	# 	algoScoreRows.append(row2)
